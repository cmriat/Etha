...
class TensorBusAgent:
    ...
    def _handle_command(self, command):
        ...
        try:
            ...
            logger.error(f"Daemon {self.rank}: Error handling command {type(command)}: {e}")
    ...
    def _handle_transfer(self, msg: Transfer):
        ...
        while ready_count < self.world_size:
            ...
            if ready_count < self.world_size:
                logger.debug(f"Daemon {self.rank}: Transfer ready progress: {ready_count}/{self.world_size}")
                time.sleep(TIME_INTERVAL)
    ...
    def _handle_query_status(self, msg: QueryStatus):
        ...
        tcpstore_state_key = f"pair:{pair_name}/rank:{self.rank}/state:{state_name}"
